...
# liga_review_approve_app.py
# 参照・修正・承認モード用アプリ
# - data/images/03_done を読む
# - data/masks/03_done の既存マスクを自動読込
# - 保存で mask / overlay / meta を更新
# - 承認で image / mask / overlay / meta を 04_approved に移動
...
class LigaReviewApproveApp:
    ...
    def __init__(self, root, inputs=None):
        self.root = root
        self.root.title("lIGA 参照・修正・承認モード")
        ...
        if self.inputs:
            ...
            messagebox.showerror("エラー", "data/images/03_done に画像がありません。")
    ...
    def _infer_stem(self) -> str:
        in_path = self.current_input_path if self.current_input_path else None
        return in_path.stem if in_path else Path(self.current_name).stem

    def _get_paths(self, stem: str, state_dirname: str, image_name: str = None):
        if image_name is None:
            image_name = f"{stem}.jpg"
        image_path = self._state_dir(IMAGES_DIR, state_dirname) / image_name
        ...
        return image_path, mask_path, overlay_path, meta_path
    ...
    def build_ui(self):
        ...
        self.lbl_info.pack(side=tk.LEFT, padx=(0, 12))

        tk.Label(left_head, text="Reviewer:").pack(side=tk.LEFT)
        self.entry_reviewer = tk.Entry(left_head, width=16)
        self.entry_reviewer.pack(side=tk.LEFT, padx=(4, 12))
        self.entry_reviewer.insert(0, self.reviewer_name)
        ...
        self.btn_next = tk.Button(right_head, text="次へ", command=self.next_image)
        self.btn_cancel = tk.Button(right_head, text="終了", command=self.force_quit)

        self.btn_cancel.pack(side=tk.RIGHT, padx=6)
        self.btn_next.pack(side=tk.RIGHT, padx=6)
        self.btn_approve.pack(side=tk.RIGHT, padx=6)
        ...
        self.brush_scale = tk.Scale(
            tools_row1, from_=5, to=100, orient=tk.HORIZONTAL, length=200,
        ...
        liga_frame.pack(side=tk.LEFT, padx=(0, 12))
        ...
        self.btn_toggle_overlay = tk.Button(
            tools_row2, text="元画像のみ", command=self.toggle_overlay
        )
        ...
        tk.Checkbutton(vis_frame, text="lIGA1", variable=self.var_show_1, command=self.on_toggle_visibility).pack(side=tk.LEFT, padx=4)
        tk.Checkbutton(vis_frame, text="lIGA2", variable=self.var_show_2, command=self.on_toggle_visibility).pack(side=tk.LEFT, padx=4)
        tk.Checkbutton(vis_frame, text="lIGA3", variable=self.var_show_3, command=self.on_toggle_visibility).pack(side=tk.LEFT, padx=4)
        tk.Checkbutton(vis_frame, text="lIGA4", variable=self.var_show_4, command=self.on_toggle_visibility).pack(side=tk.LEFT, padx=4)

        meta_frame = tk.Frame(tools_row3)
        meta_frame.pack(side=tk.LEFT, fill=tk.X, expand=True)

        self.lbl_status = tk.Label(meta_frame, text="status: -", anchor="w")
        self.lbl_status.pack(side=tk.TOP, anchor="w")

        self.lbl_meta = tk.Label(meta_frame, text="meta: -", anchor="w", justify=tk.LEFT)
        self.lbl_meta.pack(side=tk.TOP, anchor="w")
    ...
    def _write_meta(self, meta_path: Path, meta: dict):
        meta["updated_at"] = now_iso()
        with open(meta_path, "w", encoding="utf-8") as f:
            json.dump(meta, f, ensure_ascii=False, indent=2)

    def refresh_meta_labels(self):
        status = self.current_meta.get("status", "-")
        annotator = self.current_meta.get("annotator", "")
        reviewer = self.current_meta.get("reviewer", "")
        created = self.current_meta.get("created_at", "")
        updated = self.current_meta.get("updated_at", "")
        self.lbl_status.config(text=f"status: {status}")
        self.lbl_meta.config(
            text=f"annotator: {annotator}    reviewer: {reviewer}\ncreated: {created}\nupdated: {updated}"
        )
    ...
    def load_image_from_path(self, path):
        ...
        stem = Path(path).stem
        _, mask_path, _, meta_path = self._get_paths(stem, DONE_DIRNAME, image_name=Path(path).name)
        ...
        self.current_meta = self._read_meta(meta_path)
        if not self.current_meta:
            self.current_meta = {
                "image_name": Path(path).name,
                "image_relpath": str(Path(path)),
                "mask_name": mask_path.name,
                "mask_relpath": str(mask_path),
                "overlay_name": f"{stem}_overlay.png",
                "overlay_relpath": str(OVERLAYS_DIR / DONE_DIRNAME / f"{stem}_overlay.png"),
                "status": "done",
                "annotator": "",
                "reviewer": "",
                "comment": "",
    ...
    def update_header(self):
        ...
        self.lbl_info.config(text=f"{cur}/{total}: {self.current_name}")

    # ---------- render ----------
    def toggle_overlay(self):
        self.show_overlay = not self.show_overlay
        self.btn_toggle_overlay.config(text="元画像のみ" if self.show_overlay else "マスク表示")
    ...
    def update_buttons_state(self):
        ...
        enabled = tk.NORMAL if self.mask is not None else tk.DISABLED
        self.btn_save.config(state=enabled)
        self.btn_approve.config(state=enabled)
    ...
    def _should_show_brush_preview(self):
    ...
    def on_pan_drag(self, event):
        if not self.move_mode or not self._panning or self.image is None:
            return
    ...
    def save_review(self):
        if self.image is None or self.mask is None or self.current_input_path is None:
            return

        self.reviewer_name = self.entry_reviewer.get().strip() or "maki"

        stem = self.current_input_path.stem
        image_path, mask_path, overlay_path, meta_path = self._get_paths(
            stem, DONE_DIRNAME, image_name=self.current_input_path.name
        )
        ...
        try:
            ...
            meta["reviewer"] = self.reviewer_name
            meta["status"] = "reviewed"
            meta["image_relpath"] = str(image_path)
            meta["mask_relpath"] = str(mask_path)
            meta["overlay_relpath"] = str(overlay_path)

            self._write_meta(meta_path, meta)
            self.current_meta = meta
            self.refresh_meta_labels()

            messagebox.showinfo("保存完了", f"修正内容を保存しました。\n{mask_path.name}")
    ...
    def approve_current(self):
        ...
        ok = messagebox.askyesno("確認", f"{self.current_name} を承認して 04_approved に移動しますか？")
        if not ok:
            return
        ...
        stem = self.current_input_path.stem
        src_image, src_mask, src_overlay, src_meta = self._get_paths(
            stem, DONE_DIRNAME, image_name=self.current_input_path.name
        )
        dst_image, dst_mask, dst_overlay, dst_meta = self._get_paths(
            stem, APPROVED_DIRNAME, image_name=self.current_input_path.name
        )
        ...
        try:
            ...
            if src_overlay.exists():
                shutil.move(str(src_overlay), str(dst_overlay))
            if src_meta.exists():
                meta = self._read_meta(src_meta)
                meta["status"] = "approved"
                meta["reviewer"] = self.entry_reviewer.get().strip() or "maki"
                meta["approved_at"] = now_iso()
                meta["image_relpath"] = str(dst_image)
                meta["mask_relpath"] = str(dst_mask)
                meta["overlay_relpath"] = str(dst_overlay)
                with open(src_meta, "w", encoding="utf-8") as f:
                    json.dump(meta, f, ensure_ascii=False, indent=2)
                shutil.move(str(src_meta), str(dst_meta))

            messagebox.showinfo("承認完了", f"承認済みに移動しました。\n{dst_image.name}")

            current_name = self.current_input_path.name
            self.inputs = [p for p in self.inputs if Path(p).name != current_name]

            if self.cur_index >= len(self.inputs):
                self.cur_index = max(0, len(self.inputs) - 1)

            if self.inputs:
                self.load_current_input()
            else:
                messagebox.showinfo("完了", "承認対象の画像がなくなりました。")
                self.finish_and_exit()

        except Exception as e:
            messagebox.showerror("承認エラー", f"承認処理に失敗しました。\n{e}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="lIGA 参照・修正・承認モード")
    parser.add_argument("--input", "-i", nargs="+", help="入力画像パス（複数可・ワイルドカード可）")
    parser.add_argument("--input-dir", "-id", type=str, help="参照フォルダ")
...
def main():
    ...
    if args.input:
        ...
        inputs = expand_inputs_from_dir(args.input_dir)
    else:
        inputs = expand_inputs_from_dir(str(IMAGES_DIR / DONE_DIRNAME))
    ...
    _ = LigaReviewApproveApp(root, inputs=inputs)
    root.mainloop()
